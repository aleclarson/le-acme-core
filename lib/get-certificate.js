...
var optionTypes = {
  subject: certUtils.subjectTypes,
  domains: 'array',
  ...
  setChallenge: 'function', // (domain, token, keyAuthorization, done)
  ...
  return function getCertificate(options) {
    ...
    function nextDomain() {
      if (state.domains.length) {
        ...
        return certUtils.req({
          key: RSA.exportPrivatePem(options.domainKeyPair),
          format: 'DER',
          domains: state.validatedDomains,
          subject: options.subject,
        }).then(function(csr) {
          log('debug', 'Fetching new certificate: ' + state.newCertUrl);
          return acme.post(state.newCertUrl, {
            resource: 'new-cert',
            csr: RSA.toWebsafeBase64(csr.toString('base64')),
...
function parseBody(res) {
  ...
  if (!res.success) {
    if (Buffer.isBuffer(body)) {
      ...
      error = new Error("Request failed: " + JSON.stringify(body, null, 2));
    }
